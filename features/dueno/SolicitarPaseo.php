...
require_once __DIR__ . '/../../src/Models/Canje.php';
...
<body class="page-solicitar-paseo page-no-topbar-margin">
                    ...
                    <div class="section-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="canje_id" name="canje_id">
                                <option value="">No usar recompensa</option>

                                <?php foreach (($canjesPendientes ?? []) as $c): ?>
                                    <?php
                                    $tipo  = (string)($c['tipo_descuento'] ?? 'PORCENTAJE');
                                    $valor = (int)($c['valor_descuento'] ?? 0);
                                    $label = (string)($c['titulo'] ?? 'Recompensa');

                                    if ($tipo === 'PORCENTAJE') $label .= " — {$valor}% OFF";
                                    elseif ($tipo === 'FIJO')   $label .= " — ₲" . number_format($valor, 0, ',', '.') . " OFF";
                                    elseif ($tipo === 'GRATIS') $label .= " — Paseo GRATIS";
                                    ?>
                                    <option
                                        value="<?= (int)$c['canje_id'] ?>"
                                        data-tipo="<?= h($tipo) ?>"
                            ...
                            <small class="text-muted d-block mt-1">
                                Si canjeaste puntos, tu recompensa aparece aquí hasta que la uses.
                            </small>
                        </div>
    ...
    <script>
        ...
        function getRecompensaSel() {
            const sel = document.getElementById('canje_id');
            const opt = sel?.selectedOptions?.[0];
            if (!opt || !opt.value) return null;

            return {
                id: Number(opt.value || 0),
                tipo: String(opt.dataset.tipo || ''),
                valor: Number(opt.dataset.valor || 0)
            };
        }
        ...
        function recalcularTotal() {
            ...
            const r = getRecompensaSel();
            let descuento = 0;
            let extra = '';

            if (r && subtotal > 0) {
                if (r.tipo === 'PORCENTAJE') {
                    descuento = subtotal * (r.valor / 100);
                    extra = ` | Recompensa: -${r.valor}%`;
                } else if (r.tipo === 'FIJO') {
                    descuento = r.valor;
                    extra = ` | Recompensa: -${formatGs(r.valor)}`;
                } else if (r.tipo === 'GRATIS') {
                    descuento = subtotal;
                    extra = ` | Recompensa: Paseo GRATIS`;
                }
            }

            const total = Math.max(0, Math.round(subtotal - descuento));

            document.getElementById('totalEstimado').textContent = formatGs(total);
            document.getElementById('detalleTotal').textContent = detalle + extra;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            ['mascota_id_1', 'mascota_id_2', 'paseador_id', 'duracion', 'canje_id'].forEach(id => {
