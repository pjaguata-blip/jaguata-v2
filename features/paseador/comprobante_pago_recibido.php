...
// Seguridad: el comprobante del paseador solo si él es el paseador del paseo
if ((int)$data['paseador_id'] !== (int)Session::getUsuarioId()) {
...
<body>
    ...
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="h4 mb-0">Comprobante de pago recibido</h1>
            <div class="no-print d-flex gap-2">
                <button class="btn btn-outline-secondary btn-sm" onclick="window.print()">Imprimir</button>
                <a class="btn btn-primary btn-sm" href="comprobante_pago_recibido_pdf.php?pago_id=<?= (int)$pagoId ?>">Descargar PDF</a>
            </div>
        </div>
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="row g-2 small text-white p-3 rounded" style="background:#343a40">
                    ...
                    <div class="col-md-6"><strong>Fecha del registro:</strong> <?= htmlspecialchars($fechaPago) ?></div>
                    <?php if ($ref): ?>
                        <div class="col-12"><strong>Referencia:</strong> <?= htmlspecialchars($ref) ?></div>
                    <?php endif; ?>
                </div>
                <div class="mt-3 small text-muted">
                    Comprobante generado para el paseador <strong><?= htmlspecialchars($data['paseador_nombre']) ?></strong>.
                </div>
            </div>
        </div>
    </div>
