...
class PrecioNivel extends BaseModel
{
    ...
    {
        $sql = "SELECT DISTINCT tipo_servicio 
                FROM {$this->table} 
    ...
    public function crearPrecio(
        ...
        float $precioBase,
        float $precioHora,
        float $descuentoFinde = 0,
        float $descuentoFeriado = 0
    ): int {
        $data = [
            'zona'                  => $zona,
    ...
    public function actualizarPrecio(
        ...
        float $precioBase,
        float $precioHora,
        ?float $descuentoFinde = null,
        ?float $descuentoFeriado = null
    ): bool {
        $data = [
            'precio_base' => $precioBase,
            'precio_hora' => $precioHora,
        ...
        $sql = "SELECT p.*, 
                ...
                LEFT JOIN paseos ps 
                       ON p.zona = (
                           SELECT zona 
                           FROM paseadores 
                           WHERE paseador_id = ps.paseador_id
                       )
